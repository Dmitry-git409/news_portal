...
class Author(models.Model):
    to_user = models.OneToOneField(User, on_delete=models.PROTECT, unique=True)
    ...
    def __str__(self):
        return f'username: {self.to_user.username}, rating: {self.rating}'
...
class Category(models.Model):
    ...
    def __str__(self):
        return f'{self.category_field}:{self.pk}'


class Post(models.Model):
    SELECTOR = [('PO', 'Post'), ('NW', 'News')]
    to_author = models.ForeignKey(Author, on_delete=models.PROTECT)
...
class Comment(models.Model):
    to_posts = models.ForeignKey(Post, on_delete=models.PROTECT)
